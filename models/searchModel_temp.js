  ...
  for (let i = 0; i < productDetail.length; i++) {
    ...
    let app = await knex('store_products')
      ...
      .select('store_products.store_id', 'store_products.stock', 'product_varient.varient_id', 'product_varient.description', 'store_products.price', 'store_products.mrp',
        'product_varient.varient_image', 'product_varient.unit', 'product_varient.quantity', knex.raw('CASE WHEN store_products.mrp > 0 THEN 100-((store_products.price*100)/store_products.mrp) ELSE 0 END as discountper'))
...
const getSearchbystore = async (appDetatils) => {
  ...
  // Handle recent search (keep this as-is, it's not a performance bottleneck)
  if (device_id) {
    try {
      const check = await knex('recent_search').where('device_id', device_id);
      const checkww = await knex('recent_search').where('device_id', device_id).first();
      await knex('recent_search').where('keyword', keyword).delete();
      if (check.length >= 10 && checkww) {
        await knex('recent_search').where('id', checkww.id).delete();
      }
      await knex('recent_search').insert({ device_id: device_id, keyword: keyword });
    } catch (error) {
      // Silent fail
    }
  }
  ...
  if (user_id && keyword) {
    try {
      const check = await knex('recent_search').where('user_id', user_id);
      const checkww = await knex('recent_search').where('user_id', user_id).first();
      await knex('recent_search').where('keyword', keyword).delete();
      if (check.length >= 10 && checkww) {
        await knex('recent_search').where('id', checkww.id).delete();
      }
      await knex('recent_search').insert({ user_id: user_id, keyword: keyword });
    } catch (error) {
      // Silent fail
    }
  }
  ...
  productDetail.forEach(p => {
    if (p.fcat_id) {
      p.fcat_id.split(',').forEach(id => fcatIdsSet.add(parseInt(id)));
    }
  });
...
const getSearchbyBrands = async (appDetatils) => {
  ...
  if (keyword && (device_id || user_id)) {
    try {
      if (device_id && !user_id) {
        // Device-based recent search
        const check = await knex('recent_search')
          .where('device_id', device_id)
          .orderBy('id', 'asc');

        // Delete entries with same keyword
        await knex('recent_search')
          .where('keyword', keyword)
          .where('device_id', device_id)
          .delete();

        // If 10 or more, delete oldest
        if (check.length >= 10 && check[0]) {
          await knex('recent_search')
            .where('id', check[0].id)
            .delete();
        }
        ...
        await knex('recent_search')
          .insert({
            device_id: device_id,
            keyword: keyword,
            search_date: knex.fn.now()
          });
      } else if (user_id) {
        // User-based recent search
        const check = await knex('recent_search')
          .where('user_id', user_id)
          .orderBy('id', 'asc');
      ...
      // Silently handle errors for recent search
